<?php

require_once("Auth.php");


class GalaxyClient
{
	private $auth;
	private $api;
	private $headers;
	private $apiUrl = "https://galaxy.bipgame.io/api";
	private $cookies = "";
	
	public function __construct($api)
	{
		$this->api = $api;
		$this->auth = new Auth();
	}
	
	public function Auth($login, $password)
	{
		$this->headers = $this->auth->Login($login, $password);
	}
	
	private function getHeader()
	{
		#$this->headers = "X-CSRF-TOKEN: JfNPbbprDGEEU6OaJKhd5bLTu5qjB99KSwb15qGa\r\nCookie: laravel_token=; ";

		return "Content-Type: application/json
Connection: keep-alive
Accept: application/json, text/plain, */*
Accept-Language: ru-RU,ru;q=0.9,en-US;q=0.8,en;q=0.7
User-Agent: Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/81.0.4026.0 Safari/537.36
Accept: application/json
Host: galaxy.bipgame.io
Referer: https://galaxy.bipgame.io/
X-Requested-With: XMLHttpRequest\r\n" .
			   $this->headers;
	}
	
	public function Post($url, $data)
	{
		$options = array(
			'http' => array(
				'method'  => 'POST',
				'content' => json_encode( $data )
			)
		);
		return $this->request($url, $options);
	}
	
	public function Put($url)
	{
		$options = array(
			'http' => array(
				'method'  => 'PUT'
			)
		);
		return $this->request($url, $options);
	}
	
	public function Get($url)
	{
		$options = array(
			'http' => array(
				'method'  => 'GET'
			)
		);
		return $this->request($url, $options);
	}

	
	private function request($url, $options)
	{
		$options['http']['header'] = $this->getHeader();
		$response = "";

		$requestUrl = substr($url, 0, 4) == "http"
			? $url
			: $this->apiUrl . $url;
		$context  = stream_context_create($options);
		$result = file_get_contents($requestUrl, false, $context);
		if (!$result)
		{
			/*
			Array
			(
				[type] => 2
				[message] => file_get_contents(https://galaxy.bipgame.io/api/planet): failed to open stream: HTTP request failed! HTTP/1.1 401 Unauthorized

				[file] => M:\Projects\web\galaxy_bot\GalaxyClient.php
				[line] => 183
			)
			*/
			// on error try login and repeat the same request
			if ($error = error_get_last())
			{
				if (strpos($error['message'], '401 Unauthorized') !== false)
				{
					$this->api->Login();
					#print_r($this->getHeader());
                    error_clear_last();
					return $this->request($url, $options);
				}
				// FIXME: 400 here could be, don't handle it
				// throw new Exception("TODO: unhandled error (empty response?)");
			}
			// should be 200 OK here
		}
		$response = json_decode($result);

		return $response;
	}

	
	
	
	
	

	/*
	public function Login($credentials)
	{	
	
		// ####################################################
		// 0) get php session id
		
	
		stream_context_set_default(
			array(
				'http' => array(
					'method' => 'POST',
					#'content' => json_encode( $credentials ),
					'header' => $this->headers
				)
			)
		);
		$headers = get_headers("https://bipgame.io/public/users/login?online&domain=galaxy.bipgame.io");
		print_r($headers);

		$session_id = "gsk9i5svjde99aa3jq1ig7ojud";
		$this->cookies = "PHPSESSID_production=" . $session_id . "; ";
		#foreach ($headers as $h)
		#{
		#	if ($cookies = $this->parseCookies($h))
		#	{
		#		foreach ($cookies as $k => $v)
		#		{
		#			if ($k == "PHPSESSID_production")
		#			{
		#				$session_id = $v;
		#			}
		#			$this->setCookie($k, $v);
		#		}
		#	}
		#}
		##print_r($this->cookies);
		#die();
		
		// ####################################################
		// 1) find session id from bipgame.io
		
	
		stream_context_set_default(
			array(
				'http' => array(
					'method' => 'POST',
					'content' => json_encode( $credentials ),
					'header' => $this->headers . $this->cookies
				)
			)
		);
		$headers = get_headers("https://bipgame.io/public/users/login");
		print_r($headers);

		#$session_id = "";
		foreach ($headers as $h)
		{
			if ($cookies = $this->parseCookies($h))
			{
				foreach ($cookies as $k => $v)
				{
					#if ($k == "PHPSESSID_production")
					#{
					#	$session_id = $v;
					#}
					#$this->setCookie($k, $v);
				}
			}
		}
		
		if (!$session_id)
			die("Can not login for " . $credentials["purse"]);

		
		// ####################################################
		// 2) set galaxy session
		
		stream_context_set_default(
			array(
				'http' => array(
					'method' => 'GET',
					'header' => $this->headers . $this->cookies
				)
			)
		);
		
		$headers = get_headers("https://galaxy.bipgame.io/login?h=" . $session_id);
		print_r($headers);
		foreach ($headers as $h)
		{
			if ($cookies = $this->parseCookies($h))
			{
				foreach ($cookies as $k => $v)
				{
					$this->setCookie($k, $v);
				}
			}
		}
		$this->headers .= $this->cookies;		

		#print_r($this->headers . $this->cookies);
		#die();
		
		
		
		// 3) get X-CSRF token
		$meta = get_meta_tags("https://galaxy.bipgame.io");
		$this->headers = "X-CSRF-TOKEN: " . $meta["csrf-token"] . "\r\n" . $this->headers . "X-CSRF-TOKEN: " . $meta["csrf-token"];
print_r($meta);
		print_r($this->headers);
		#die();
	}

	
	private function parseCookies($str, $force = false)
	{
		$cookie_str = $str;
		if (!$force)
		{
			if (substr($cookie_str, 0, 12) != "Set-Cookie: ")
				return false;
			
			$cookie_str = substr($cookie_str, 12, strlen($cookie_str) - 12);
		}

		parse_str(strtr($cookie_str, array('&' => '%26', '+' => '%2B', ';' => '&')), $cookie_arr);
		return $cookie_arr;
	}
	
	private function setCookie($key, $value)
	{
		$found = false;
		if ($cookies = $this->parseCookies($this->cookies, true))
		{
			foreach ($cookies as $k => $v)
			{
				if ($k == $key)
				{
					$found = true;
					break;
				}
			}
		}
		if (!$found)
		{
			$this->cookies .= $key . ($value ? "=" . $value : "") . "; ";
		}
	}
	*/
}

